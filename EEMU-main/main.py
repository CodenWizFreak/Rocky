...
flags.DEFINE_bool(
    "distributed",
    True,
    "If true, use torch.distributed when launched via torchrun. If false, run single-process.",
)


def _get_dist_env():
    """Best-effort distributed environment detection.

    Returns:
        (use_distributed, rank, world_size, local_rank)
    """
    rank = int(os.environ.get("RANK", "0"))
    world_size = int(os.environ.get("WORLD_SIZE", "1"))
    local_rank = int(os.environ.get("LOCAL_RANK", str(rank)))

    # torchrun sets MASTER_ADDR/MASTER_PORT and usually RANK/WORLD_SIZE.
    has_rendezvous = ("MASTER_ADDR" in os.environ) and ("MASTER_PORT" in os.environ)
    has_rank_info = ("RANK" in os.environ) and ("WORLD_SIZE" in os.environ)
    use_distributed = FLAGS.distributed and (has_rank_info or has_rendezvous) and world_size >= 1
...
def main(argv):
    ...
    use_distributed, env_rank, env_world_size, local_rank = _get_dist_env()

    if use_distributed:
        backend = 'nccl' if (torch.cuda.is_available() and FLAGS.device == 'gpu') else 'gloo'
        dist.init_process_group(backend=backend)
        rank = dist.get_rank()
        # Prefer LOCAL_RANK when present (torchrun).
        local_rank = int(os.environ.get("LOCAL_RANK", str(rank)))
    else:
        rank = 0
    ...
    if rank == 0:
        ...
        init_param = [
            ...
            dataloader_val,
            inference_users_dataloader,
            inference_items_dataloader,
        ]
    ...
    if dist.is_available() and dist.is_initialized():
        dist.broadcast_object_list(init_param, src=0)
