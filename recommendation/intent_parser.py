...
liked_genres — genres/styles the user explicitly praised or requested as ongoing preference.
  "I love action movies" → ["Action"]
  "what about action films, suggest me some, I like them" → genres_requested=["Action"], liked_genres=["Action"]
  "more superhero adventure stuff" → ["Action", "Adventure"] if clearly preference.

disliked_genres — genres/styles the user explicitly rejected or blocked.
  "fuck musicals" → ["Music"]
  "no western movies" → ["Western"]
  "I hate horror" → ["Horror"]
  "western movies suck" → ["Western"]
  If user rejects a genre, NEVER also add it to genres_requested.

actors_requested — actor names as mentioned by user (any spelling)
  Only people, never teams/franchises/genres. "Avengers" is NOT an actor.
  Do NOT infer actors from a movie title the user liked unless user explicitly asks for that actor.

directors_requested — director names, extracted from phrases like:
  "directed by X", "films by X", "movies by X", "anything from X", "X's films"
  Also infer from: "I love Nolan's work" → "Christopher Nolan"
  Do NOT infer directors from a movie title the user liked unless user explicitly asks for that director.

plot_description — a FULL natural language description of the plot the user wants.
  Capture this when the user describes a scenario, theme, or story arc, e.g.:
  "movies where a man loses his memory" → "a man loses his memory"
  "something about AI taking over the world" → "AI taking over the world"
  "a story involving a heist gone wrong in space" → "a heist gone wrong in space"
  Leave as "" if no plot description was given.

plot_keywords — short thematic keywords extracted from the query (max 5).
  Only populate if plot_description is also non-empty.

liked_found — titles the user expressed POSITIVE sentiment about. Catch ALL of:
  - Direct: "I loved X", "I liked X", "I enjoyed X", "I adored X"
  - Slang: "X slapped", "X was fire", "X was peak", "X hit different", "X was a banger"
  - Indirect: "more like X" (implies X was liked), "something similar to X"
  - "X blew me away", "X was a masterpiece", "X is a classic", "obsessed with X"
  - "couldn't stop watching X", "watched X 3 times"
  - Resolve positional refs ("the second one", "that last one") using LAST_RECOMMENDATIONS
  - Do NOT copy titles from USER_PROFILE, RECENT_CONVERSATION, or LAST_RECOMMENDATIONS unless current USER_MESSAGE clearly expresses positive sentiment about them.
  - Do NOT treat recommended titles as liked just because they appeared in last recommendations.

disliked_found — titles the user expressed NEGATIVE sentiment about. Catch ALL of:
  - Direct: "I hated X", "I disliked X", "I didn't like X", "I couldn't stand X"
  - Mild: "X wasn't for me", "X wasn't my thing", "X was okay I guess" (still dislike)
  - Boredom: "X was boring", "X put me to sleep", "X was a slog", "X dragged"
  - Disappointment: "X let me down", "I expected more from X", "X didn't live up to the hype"
  - Already seen: "I've seen X already" → add to disliked_found (don't re-recommend)
  - Slang: "X was mid", "X was meh", "X was trash", "X was garbage", "hard pass on X"
  - Regret: "I wasted 2 hours on X", "I regret watching X", "I gave up on X"
  - Resolve positional refs using LAST_RECOMMENDATIONS
  - Do NOT copy old disliked titles from USER_PROFILE/RECENT_CONVERSATION unless current USER_MESSAGE mentions or refers to them.
  - If user says "fuck musicals", output disliked_genres=["Music"], not stale disliked_found.

sentiment_last_recs — overall feeling about the LAST SHOWN RECOMMENDATIONS (not a specific movie):
  "positive"  — user likes the suggestions in general
  "negative"  — user dislikes the suggestions in general ("these are boring", "not what I wanted")
  "neutral"   — mixed or unclear
  null        — no sentiment expressed about the last batch

wants_different — true if user wants a fresh set of results different from the last batch.
  Examples: "something different", "try again", "none of these", "can you do better",
  "not feeling it", "show me other options", "I've seen all of these"
