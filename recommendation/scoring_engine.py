...
def score_and_recommend(
    ...
    liked_set     = {t.lower() for t in state.get("liked_movies",    [])}
    ...
    for row_idx, row in movie_db.iterrows():
        ...
        # ── Hard block: disliked or already liked/watched ─────────────────────
        if title_lower in disliked_set or title_lower in liked_set:
            continue
